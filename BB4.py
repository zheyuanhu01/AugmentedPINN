...
class PINN_XPINN:
    ...
    def _residual(self, u_net_, v_net_, xf_, tf_):
        ...
        f = self.PDE1(u_, u_t, u_x, v_x) + self.PDE2(u_, v_, v_t, u_x, v_x, u_xxx) 
        return f
    ...
    def get_loss_xpinn(self):
        ...
        # Residual/collocation loss
        f1 = self._residual(self.u1_net, self.v1_net, self.xf1, self.tf1)
        f2 = self._residual(self.u2_net, self.v2_net, self.xf2, self.tf2)
        f3 = self._residual(self.u3_net, self.v3_net, self.xf3, self.tf3)
        f4 = self._residual(self.u4_net, self.v4_net, self.xf4, self.tf4)
        mse_f = f1.square().mean() + f2.square().mean() + f3.square().mean() + f4.square().mean()
