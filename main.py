...
parser = argparse.ArgumentParser(description='Augmented PINN')
parser.add_argument('--SEED', type=int, default=0) # Random seed
parser.add_argument('--dataset', type=str, default='Wave') # 'burgers', 'helmholtz', 'KG', 'Wave'; Choose the PDE here 
parser.add_argument('--PINN', type=bool, default=False) # Train or not?
parser.add_argument('--XPINN', type=bool, default=False)
parser.add_argument('--SXPINN', type=bool, default=False) # SXPINN is the APINN model we propose
parser.add_argument('--SXPINN_Prior', type=str, default="XPINN") # XPINN, MPINN, PINN
parser.add_argument('--device', type=str, default="cuda")
parser.add_argument('--epochs', type=int, default=100000)
parser.add_argument('--lr', type=float, default=8e-4)

parser.add_argument('--PINN_h', type=int, default=20) # width of PINN
parser.add_argument('--PINN_L', type=int, default=10) # layers of PINN
parser.add_argument('--XPINN_h', type=int, default=20) # width of XPINN
parser.add_argument('--XPINN_L', type=int, default=6) # layers of XPINN

parser.add_argument('--SXPINN_G_h', type=int, default=20) # width of G net in APINN
parser.add_argument('--SXPINN_G_L', type=int, default=2) # layers of G net in APINN
parser.add_argument('--SXPINN_E_h', type=int, default=20) # width of E net in APINN
parser.add_argument('--SXPINN_E_L', type=int, default=4) # layers of E net in APINN
parser.add_argument('--SXPINN_h_h', type=int, default=20) # width of h net in APINN
parser.add_argument('--SXPINN_h_L', type=int, default=3) # layers of h net in APINN
parser.add_argument('--SXPINN_gate_trainable', type=bool, default=True)
# If dataset is Helmholtz, pls choose APINN use --SXPINN_h_L=2 --SXPINN_E_L=5 --SXPINN_h_h=8 lbfgs 100k 5e-1
# Since APINN is a generalization of PINN and XPINN, you can always adjust its structure for better performance.

parser.add_argument('--PINN_weights', type=list, default=[1, 20]) # residual and boundary loss weights
...
class Net(nn.Module): # multi-layer fully connected network
...
class Gate_Net(nn.Module): # Gate net of APINN
...
class SXPINN_Net(nn.Module): # APINN model
...
class PINN_XPINN:
    def __init__(self):
        ...
        self.xpinn_w_ra = args.XPINN_weights[4] # additional residual interface for XPINNv2
        ...
        if args.dataset in INIT_DATASETS: # intial points for the initial conditions
            # PINN
    ...
    def get_loss_pinn(self):
        ...
        # Sum losses
        loss = self.pinn_w_b*mse_ub + self.pinn_w_rc*mse_f
